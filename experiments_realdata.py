...
#%%
def main(datasets, paras, opt, repeat_time):
    # repeat_time = 20
    ...
    for data in datasets:
        ...
        for k in range(repeat_time):
            ...
            for opt['epsilon'], opt['tol'] in product(*param_values):
                ...
                cross_err_1, test_err_1 = result_test['cross_err'], result_test['test_err']
...
if __name__ == "__main__":
    ...
    parser.add_argument("--solvers", default = "open_source",
                        help = "choose slovers")
    parser.add_argument("--data_scale", default = "moderate",
                        help = "choose datasets for test")
    parser.add_argument("--repeat_time", type = int, default = 20,
                        help = "define the repeat time")
    args = parser.parse_args()
    if args.data_scale == "large":
        datasets = ['mushrooms', 'phishing']
        paras = dict(epsilon = [0., 1e-2, 1e-4], tol = [1e-2])
    if args.solvers == "GUROBI":
        opt["svm_solver"], opt["main_solver"] = cp.GUROBI, cp.GUROBI
    repeat_time = args.repeat_time
    main(datasets, paras, opt, repeat_time)
